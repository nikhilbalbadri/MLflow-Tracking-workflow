...
if __name__ == "__main__":
    ...
    feature_count = int(sys.argv[4])if len(sys.argv) > 4 else 0
    ...
    with mlflow.start_run():
        if regression_model == "linear":
            ...
            modelName = "Ridge"
            model = Ridge(alpha=alpha, fit_intercept=intercept, random_state=20)
        else:
            modelName = "Lasso"
            model = Lasso(alpha=alpha, fit_intercept=intercept, random_state=20)
